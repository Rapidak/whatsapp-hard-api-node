...
const Disconnect = require('./resDisconnectReason');
...
class WhatsAppInstance {
	...
	setHandler() {
		...
		sock?.ev.on('connection.update', async (update) => {
			...
			await Disconnect.lastDisconnect(this.key, lastDisconnect);
	...
	async sendMediaFile(data, origem) {
		...
		if (origem === 'url') {
			...
			if (!data.filename || data.filename === '') { throw new Error('Nome do arquivo é obrigatorio'); }
